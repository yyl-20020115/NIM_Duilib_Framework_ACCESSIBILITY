...
LRESULT Window::DoHandlMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& handled)
{
	...
	case WM_GETOBJECT:
	{
		if(static_cast<long>(lParam) == static_cast<long>(OBJID_CLIENT)) {
			//IACCESSIBLE
			handled = TRUE;
			if (m_pAccessible == NULL) {
				m_pAccessible = GetAccessible();
			}
			if (m_pAccessible != NULL) {
				// If the accessible object already exists, return it
				if (m_pAccessible->GetWindowHandle() == this->m_hWnd) {
					IUnknown* pUnknown = NULL;
					if (S_OK == m_pAccessible->QueryInterface(IID_IUnknown, (void**)&pUnknown))
			...
			return AccessibleObjectFromWindow(m_hWnd, OBJID_CLIENT, IID_IAccessible, (void**)&m_pAccessible);
		}
		else
		if (static_cast<long>(lParam) == static_cast<long>(UiaRootObjectId) 
			&& GlobalManager::IsAutomationEnabled()) {
			handled = TRUE;
