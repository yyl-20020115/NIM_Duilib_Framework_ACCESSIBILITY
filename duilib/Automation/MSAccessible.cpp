...
namespace ui
{
	...
	MSAccessible::MSAccessible(MSAccessible* pParent)
		: m_hWnd(nullptr)
		, m_pParent(pParent)
		, m_refCount(1)
		, m_children()
	...
	MSAccessible* MSAccessible::GetParent() const
	{
		return m_pParent;
	}
	...
	HRESULT STDMETHODCALLTYPE MSAccessible::Next(ULONG celt, VARIANT* rgVar, ULONG* pCeltFetched)
	{
		...
		// 枚举子元素
		while (fetched < celt && m_enumPos < total) {
			VariantInit(&rgVar[fetched]);
			MSAccessible* pChild = m_children[m_enumPos];
			if (pChild) {
		...
		// 如果获取的数量小于请求数量，返回S_FALSE，否则S_OK
	...
	HRESULT STDMETHODCALLTYPE MSAccessible::get_accChild(VARIANT varChild, IDispatch** ppdispChild)
	{
		...
		*ppdispChild = nullptr;
		...
		MSAccessible* pChild = m_children[index];
		if (!pChild)
			return S_FALSE;
	...
	HRESULT STDMETHODCALLTYPE MSAccessible::get_accRole(VARIANT varChild, VARIANT* pvarRole)
	{
		...
		if (m_hWnd) {
			...
			else if (wcscmp(className, L"RichEdit20A") == 0
	...
	HRESULT STDMETHODCALLTYPE MSAccessible::get_accKeyboardShortcut(VARIANT varChild, BSTR* pszKeyboardShortcut)
	{
		...
		*pszKeyboardShortcut = nullptr;
		...
		// 获取窗口文本
		WCHAR szText[1024] = { 0 };
		::GetWindowText(m_hWnd, szText, _countof(szText));

		// 查找助记符（&后跟字符）
		for (int i = 0; szText[i]; ++i) {
			if (szText[i] == L'&' && szText[i + 1] && szText[i + 1] != L'&') {
				WCHAR shortcut[3] = { L'\0' };
