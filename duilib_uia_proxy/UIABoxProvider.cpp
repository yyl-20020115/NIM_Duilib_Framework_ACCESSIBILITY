	...
	*pFound = nullptr;
	...
	if (propertyId == 0) {
		if (pStartAfter == nullptr) {
			IAccessible* pFirst = nullptr;
			VARIANT varChild;
			VariantInit(&varChild);
			varChild.vt = VT_I4;
			varChild.llVal = 0;
			if (S_OK == m_pControl->get_accChild(varChild, (IDispatch**) & pFirst)) {
				if (pFirst)
					*pFound = static_cast<IRawElementProviderSimple*>(
						pFirst->GetUIAProvider());
			}
		}
		else {
	...
	// the Name, AutomationId, and IsSelected properties.
		...
		case UIA_AutomationIdPropertyId:
		{
			...
			HRESULT hr = m_pControl->get_accChild(value, (IDispatch**)&child);
			if (hr != S_OK || !child)
				return UIA_E_ELEMENTNOTAVAILABLE;
...
IFACEMETHODIMP UIAControlProvider::Scroll(ScrollAmount horizontalAmount, ScrollAmount verticalAmount) { UIA_CHECK_ELEMENT_RETURN(m_pControl); }
